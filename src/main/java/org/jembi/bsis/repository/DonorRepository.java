...
public class DonorRepository {
	...
	public Donor findDonorByDonationIdentificationNumber(String donationIdentificationNumber) throws NoResultException {
		return em
				.createNamedQuery(DonorNamedQueryConstants.NAME_FIND_DONOR_BY_DONATION_IDENTIFICATION_NUMBER,
						Donor.class)
				.setParameter("donationIdentificationNumber", donationIdentificationNumber)
				.setParameter("excludedStatuses", Arrays.asList(DonorStatus.MERGED))
				.setParameter("isDeleted", Boolean.FALSE).setParameter("isDonationDeleted", Boolean.FALSE)
				.getSingleResult();
	}

	public List<Donor> findAnyDonor(String firstName, String middleName, String lastName, boolean usePhraseMatch) {
		...
		if (!usePhraseMatch) {
			...
			if (StringUtils.isNotBlank(middleName)) {
				...
				expressions.add(builder.equal(root.<String>get("middleName"), middleName));
			}
			...
			if (StringUtils.isNotBlank(firstName)) {
				...
				expressions.add(
						builder.like(root.<String>get("firstName"), createLikeExpression(firstName, donorSearchMode)));
			}
			if (StringUtils.isNotBlank(middleName)) {
				// Match on similar first name
				expressions.add(builder.like(root.<String>get("middleName"),
						createLikeExpression(middleName, donorSearchMode)));
			}
			if (StringUtils.isNotBlank(lastName)) {
				// Match on similar last name
				expressions.add(
						builder.like(root.<String>get("lastName"), createLikeExpression(lastName, donorSearchMode)));
			}
		}
	...
	public List<PreferredLanguage> getAllLanguages() {
		//TypedQuery<PreferredLanguage> query = em.createQuery("SELECT l FROM PreferredLanguage l",
			//	PreferredLanguage.class);
		TypedQuery<PreferredLanguage> query = em.createQuery("SELECT l FROM PreferredLanguage l where preferredLanguage='English'",
				PreferredLanguage.class);
		return query.getResultList();
	}

	public List<IdType> getAllIdTypes() {
		TypedQuery<IdType> query = em.createQuery("SELECT id FROM IdType id", IdType.class);
		return query.getResultList();
	}
	...
	public List<AddressType> getAllAddressTypes() {
		TypedQuery<AddressType> query = em.createQuery("SELECT addressType FROM AddressType addressType",
				AddressType.class);
		return query.getResultList();
	...
	public DonorSummaryViewModel findDonorSummaryByDonorNumber(String donorNumber) throws NoResultException {
		return em.createQuery(
				"SELECT NEW org.jembi.bsis.viewmodel.DonorSummaryViewModel(d.id, d.firstName, d.middleName, d.lastName, d.gender, d.birthDate) "
						+ "FROM Donor d " + "WHERE d.donorNumber = :donorNumber " + "AND d.isDeleted = :isDeleted "
						+ "AND d.donorStatus NOT IN :excludedStatuses ",
				DonorSummaryViewModel.class).setParameter("donorNumber", donorNumber).setParameter("isDeleted", false)
				.setParameter("excludedStatuses", Arrays.asList(DonorStatus.MERGED)).getSingleResult();
	}
	...
	public List<Donor> getDuplicateDonors(String firstName, String lastName, Date birthDate, Gender gender) {
		return em.createNamedQuery(DonorNamedQueryConstants.NAME_GET_DUPLICATE_DONORS, Donor.class)
				...
				.setParameter("birthDate", birthDate).setParameter("gender", gender).getResultList();
	}
	...
	public List<MobileClinicDonorDTO> findMobileClinicDonorsByVenues(Set<UUID> venueIds) throws NoResultException {
		...
		// using empty list as a parameter causes a bad SQL Grammar exception. If the
		// list is empty we have to
		// override the parameter to null.
		if (venueIds != null && venueIds.isEmpty()) {
			venueIds = null;
		}
		return em
				.createNamedQuery(DonorNamedQueryConstants.NAME_FIND_MOBILE_CLINIC_DONORS_BY_VENUES,
						MobileClinicDonorDTO.class)
				.setParameter("isDeleted", false).setParameter("excludedStatuses", Arrays.asList(DonorStatus.MERGED))
				.setParameter("venueIds", venueIds).setParameter("includeAllVenues", includeAllVenues).getResultList();
	}
	...
	public boolean verifyDonorExists(UUID id) {
		Long count = em.createNamedQuery(DonorNamedQueryConstants.NAME_COUNT_DONOR_WITH_ID, Long.class)
				.setParameter("id", id).setParameter("excludedStatuses", Arrays.asList(DonorStatus.MERGED))
	...
	// Added by Kaleb
	public List<Donor> findDonorByPhoneNumber(String phoneNumber) throws NoResultException {
		return em.createNamedQuery(DonorNamedQueryConstants.NAME_FIND_DONOR_BY_PHONE_NUMBER, Donor.class)
				.setParameter("phoneNumber", phoneNumber)
				.setParameter("excludedStatuses", Arrays.asList(DonorStatus.MERGED))
				.setParameter("isDeleted", Boolean.FALSE).getResultList();
	}

	public List<Donor> findUpdatedDonors(Date startDate, Date endDate) {
		// modificationTracker.lastUpdated
		String queryString = "SELECT d FROM Donor " + "d LEFT JOIN fetch d.donations donations "
				+ "WHERE d.isDeleted = :isDeleted " + "and d.donorStatus not in :donorStatus " + "and "
				+ "((d.modificationTracker.lastUpdated >= :startDate "
				+ "and d.modificationTracker.lastUpdated < :endDate )" + "or "
				+ "(donations.modificationTracker.lastUpdated >= :startDate "
				+ "and donations.modificationTracker.lastUpdated < :endDate )" + ")";
		...
		queryString = "SELECT d FROM Donor " + "d LEFT JOIN fetch d.deferrals deferrals "
				+ "WHERE d.isDeleted = :isDeleted " + "and d.donorStatus not in :donorStatus " + "and "
				+ "deferrals.modificationTracker.lastUpdated >= :startDate "
				+ "and deferrals.modificationTracker.lastUpdated < :endDate ";
		query = em.createQuery(queryString, Donor.class);
		query.setParameter("isDeleted", Boolean.FALSE);
		query.setParameter("startDate", startDate);
		query.setParameter("endDate", endDate);
		query.setParameter("donorStatus", Arrays.asList(DonorStatus.MERGED));
		List<Donor> donorsWithDeferal = query.getResultList();
		for (Donor i : donorsWithDeferal) {
			boolean addDonor = true;
			for (Donor j : donors) {
